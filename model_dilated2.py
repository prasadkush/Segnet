...
class ConvLayer(nn.Module):
    def __init__(self, inputfeatures, outputinter, kernel_size=7, stride=1, padding=3,  output=64, layertype=1, droupout=False):
...
class SegmentationDilated(nn.Module):
    def __init__(self, kernel1_size=7, kernel2_size=3, kernel3_size=5, padding=3, out_channels=12):
        ...
        self.layer4 = DilationModule(128, module_type=1, output7=96, output3=96, output5=0, kernel1_size=kernel1_size, padding1=3, kernel2_size=3, padding2=2, dilation2=2, kernel3_size=5, padding3=4, dilation3=2)
        self.layer5 = DilationModule(192, module_type=1, output7=96, output3=96, output5=0, kernel1_size=kernel1_size, padding1=3, kernel2_size=3, padding2=4, dilation2=4, kernel3_size=5, padding3=8, dilation3=4)
        self.layer6 = DilationModule(192, module_type=1, output7=96, output3=96, output5=0, kernel1_size=kernel1_size, padding1=3, kernel2_size=3, padding2=6, dilation2=6, kernel3_size=5, padding3=12, dilation3=6)

        self.layer7 = nn.Sequential(nn.Conv2d(192, 128, 1, stride=1, padding=0), 
        ...
        #    nn.BatchNorm2d(128),
           #nn.ReLU())
        #    nn.PReLU(num_parameters=1, init=0.25, device=None, dtype=None))
        self.layer8 = ConvLayer(128, 128, kernel_size=5, stride=1, padding=2,  output=128, layertype=3, droupout=True)
        self.layer9 = ConvLayer(128, 64, kernel_size=5, stride=1, padding=2,  output=64, layertype=3, droupout=True)
        self.layer10 = ConvLayer(64, 64, kernel_size=5, stride=1, padding=2,  output=64, layertype=3, droupout=True)
        self.ClassifyBlock = ClassifyBlock(64, out_channels)

    def forward(self, x):
        outsize0 = x.shape
        #start_time = time()
        out1, inds1 = self.layer1(x)
        #end_time = time()
